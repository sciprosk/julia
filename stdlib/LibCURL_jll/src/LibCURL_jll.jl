...
const libcurl_path = LazyStringFunc(() -> joinpath(dirname(Sys.BINDIR), libcurl_name))
const libcurl = LazyLibrary(libcurl_path; dependencies=[libssh2, libnghttp2],
                            on_load_callback = (hdl) -> begin
                                @assert ccall(dlsym(hdl, :curl_global_init), UInt32, (Clong,), 0x03) == 0
