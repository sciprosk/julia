...
@testset "LazyLibrary" begin; mktempdir() do dir
    ...
    global libccalllazyfoo = LazyLibrary(lclf_path; on_load_callback=hdl -> global lclf_loaded = true)
    global libccalllazybar = LazyLibrary(lclb_path; dependencies=[libccalllazyfoo], on_load_callback=hdl -> global lclb_loaded = true)
    ...
    calc_libname() = joinpath(private_libdir, "libccalllazyfoo.$(Libdl.dlext)")
    lazy_name_lazy_lib = LazyLibrary(LazyStringFunc(calc_libname))
    @test dlpath(lazy_name_lazy_lib) == realpath(calc_libname())
end; end
