...
function lbt_onload_callback(hdl)
    # We don't use `BLAS.lbt_forward()` here because we explicitly don't
    # want to take a lock on the config cache.  We also avoid using ccall's
    # to `libblastrampoline` as that object is in the middle of being
    # loaded, we must use the provided `hdl`.
    ccall(Libdl.dlsym(hdl, :lbt_forward), Int32, (Cstring, Int32, Int32, Cstring),
    ...
    if !haskey(ENV, "OPENBLAS_NUM_THREADS") && !haskey(ENV, "GOTO_NUM_THREADS") && !haskey(ENV, "OMP_NUM_THREADS")
        ...
        ccall(Libdl.dlsym(hdl, :lbt_set_num_threads), Cvoid, (Int32,), nthreads)
    end
end

# Force `libblastrampoline` to forward to `libopenblas` once it's loaded
push!(libblastrampoline_jll.libblastrampoline.dependencies, libopenblas)
libblastrampoline_jll.libblastrampoline.on_load_callback = lbt_onload_callback
